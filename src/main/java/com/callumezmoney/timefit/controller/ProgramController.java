...
public class ProgramController {
    ...
    @PostMapping()
    public ResponseEntity<?> addProgram(@RequestBody Program newProgram, Principal principal){
        return ResponseEntity.ok(programMapper.entityToDto(programService.addProgram(newProgram, principal.getName()).get()));
    }

    @PutMapping()
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void editProgram(@RequestBody Program newProgram, Principal principal){
